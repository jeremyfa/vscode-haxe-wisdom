/*
 * Vendored from vshaxe's `vshaxe` haxelib (https://github.com/vshaxe/vshaxe,
 * haxelib/src/), rather than depending on `-lib vshaxe`: the haxelib is not
 * installed here and its published externs lag behind the vshaxe runtime we
 * actually talk to. The layout mirrors upstream exactly, so swapping these
 * files for `-lib vshaxe` later is a no-op.
 *
 * Retrieve at runtime with:
 *   var api:Vshaxe = Vscode.extensions.getExtension("nadako.vshaxe").exports;
 *
 * Note: `displayHost` / `displayPort` are assigned by vshaxe only once its own
 * language server has started, so they are null at activation time.
 */
